...
<div class="flex h-200 w-full flex-col bg-surface-low">
	<div
		class="bg-surface flex flex-col gap-4 border-b border-outline p-4 sm:flex-row sm:items-center sm:justify-between"
	>
		<div class="flex gap-2">
			{#each tabs as tab (tab.id)}
				<button
					class="rounded-md px-4 py-2 text-sm font-semibold tracking-wide transition-all {activeTab ===
					tab.id
						? 'border border-primary/50 bg-primary/20 text-primary'
						: 'border border-transparent text-foreground-secondary hover:bg-surface-highest hover:text-foreground'}"
		...
		<div class="relative flex w-full max-w-xs items-center">
			...
			<input
				...
				class="w-full rounded-full border border-outline bg-surface-lowest py-2 pr-4 pl-10 text-sm focus:border-primary focus:outline-none"
			/>
		</div>
	</div>

	<div class="flex-1 overflow-y-auto bg-surface-lowest p-4">
	...
	<div class="bg-surface flex border-t border-outline p-4">
		<button
			class="ml-auto w-64 cursor-pointer rounded-sm border border-primary/50 bg-primary/20 py-4 text-sm font-bold tracking-widest text-primary uppercase transition-all hover:bg-primary/50 hover:text-foreground active:scale-[0.98]"
			onclick={handleApply}
		>
			Apply Preferences
		</button>
