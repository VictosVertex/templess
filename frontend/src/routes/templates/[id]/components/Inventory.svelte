<script lang="ts">
    ...
    function handleToggleOptimization() {
        if (
            ...
            for (const [slot, equippedItem] of Object.entries(builder.equippedItems)) {
                if (equippedItem?.source !== EquipSource.User) continue;
                equipped_items[parseInt(slot, 10)] = equippedItem.item.id;
            }

            const request: ClientMessage = {
                type: ClientMessageType.Start,
                data: { class_id: builder.getTemplateClass().id, equipped_items, preferences: builder.preferences }
            };
    ...
    let buttonColor = $derived.by(() => {
        if (backend.optimizationStatus === OptimizationStatus.Ready || backend.optimizationStatus === OptimizationStatus.Finished) {
            return 'bg-primary border-primary/20 text-surface-lowest hover:brightness-110 hover:shadow-lg';
        }
        if (backend.optimizationStatus === OptimizationStatus.Setup || backend.optimizationStatus === OptimizationStatus.Grounding) {
...
<div class="@container relative mx-auto w-full aspect-[640/780] max-w-[min(100%,640px,calc(75vh*640/780))]">
    
    <div class="absolute inset-x-0 top-0 w-full aspect-square">    
        <div class="absolute bottom-[8%] left-1/2 flex -translate-x-1/2 flex-col items-center justify-center whitespace-nowrap">
            <span class="mb-1 text-[clamp(9px,2cqi,10px)] font-bold tracking-[0.2em] text-foreground-secondary uppercase">
                Total Utility
            </span>
            <span class="font-technical text-[clamp(14px,4cqi,18px)] font-bold text-primary">
                {builder.totalUtility.toFixed(2)}
            </span>
        </div>

        <button
            class="absolute top-1/2 left-1/2 z-10 flex w-[20cqi] aspect-square -translate-x-1/2 -translate-y-1/2 transform
        ...
        >
            <div class="absolute inset-[2px] rounded-full border-2 {isRunning ? 'animate-spin border-surface-lowest/10 border-t-surface-lowest/80 border-r-transparent' : 'border-surface-lowest/20'}"></div>
